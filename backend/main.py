...
def get_dashboard_stats(db: Session = Depends(get_db)):
    ...
    return {
        ...
        "high_similarity_alerts": high_sim,
        "distribution": {
            "high": high_sim,
            "medium": med_sim,
            "low": low_sim
...
def list_models(db: Session = Depends(get_db)):
    models_list = db.query(models.AIModel).order_by(models.AIModel.created_at.desc()).all()
    if not models_list:
        # Seed default models if empty
        defaults = [
            {"name": "Gemini 2.5 Pro", "identifier": "gemini-2.5-pro", "provider": "Google", "api_key": "", "base_url": "", "description": "多模态理解能力强, 适用于复杂场景和环境细节分析，精准扣分。", "is_default": "true"},
            {"name": "GPT-4o", "identifier": "gpt-4o", "provider": "OpenAI", "api_key": "", "base_url": "https://api.openai.com/v1", "description": "通用性强, 稳定可靠", "is_default": "false"},
            {"name": "MiniMax 2.7", "identifier": "minimax-2.7", "provider": "MiniMax", "api_key": "", "base_url": "https://api.minimax.chat/v1", "description": "国产优秀大模型", "is_default": "false"},
            {"name": "Qwen3-VL-Plus", "identifier": "qwen-vl-plus", "provider": "Alibaba", "api_key": "", "base_url": "https://dashscope.aliyuncs.com/compatible-mode/v1", "description": "视觉理解能力极强", "is_default": "false"}
        ]
        for m in defaults:
